...
- **The Python School:** ✅ **shipped.** A full, self-contained 68-lesson course
  under `docs/python/`, generated by `tools/pybuild.py` and verified by
  `tools/pyverify.py` (which runs every example and checks its output, the Python
  analogue of "compile it first"). Base Camp through Level 6 "Build Your Own Jarvis",
  real CPython in the browser via Pyodide, a Snake Pit of verified puzzles, an Insult
  Compiler for error-message literacy, a ten-project workshop, quizzes, an 80-term
  glossary, and cheat sheets. Shares accounts and progress with the Rust school. It
  leans into contrast with Rust throughout ("here is when to pick which", and Level
  51 shows calling Rust from Python) rather than teaching in isolation.
